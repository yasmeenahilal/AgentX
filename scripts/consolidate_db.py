    ...
    with Session(engine) as session:
        for vdb in vector_dbs:
            ...
            statement = select(VectorDB).where(
                ...
                VectorDB.index_name == vdb["index_name"]
            )
            existing = session.exec(statement).first()
            
            if existing:
                logger.info(f"Vector DB {vdb['index_name']} for user {vdb['user_id']} already exists in new table")
                continue
            ...
            logger.info(f"Migrated vector_db record: {vdb['index_name']} for user {vdb['user_id']}")
...
def migrate_pinecone_db():
    ...
    with Session(engine) as session:
        for pdb in pinecone_dbs:
            ...
            if not vector_db:
                logger.warning(f"No matching vectordb found for pinecone_db {pdb['index_name']}")
                continue
            
            # Check if this record already exists in pineconedb
            statement = select(PineconeDB).where(PineconeDB.vector_db_id == vector_db.id)
            existing = session.exec(statement).first()
            
            if existing:
                logger.info(f"Pinecone DB for index {pdb['index_name']} already exists in new table")
                continue
...
def migrate_faiss_db():
    ...
    with Session(engine) as session:
        for fdb in faiss_dbs:
            ...
            if not vector_db:
                logger.warning(f"No matching vectordb found for faiss_db {fdb['index_name']}")
                continue
            ...
            if existing:
                logger.info(f"FAISS DB for index {fdb['index_name']} already exists in new table")
                continue
...
def migrate_file_uploads():
    ...
    with Session(engine) as session:
        for fu in file_uploads:
            ...
            if not vector_db:
                logger.warning(f"No matching vectordb found for file_upload {fu['file_name']}")
                continue
            ...
            if not faiss_db:
                logger.warning(f"No matching faissdb found for file_upload {fu['file_name']}")
                continue
            ...
            if existing:
                logger.info(f"File upload {fu['file_name']} already exists in new table")
                continue
