...
from models.vector_db import DBTypeEnum, FaissDB, PineconeDB, VectorDB
...
def get_index_type(
    user_id: str, index_name: str, session: Session = None
) -> Optional[str]:
    """
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            statement = select(Agent).where(
                Agent.user_id == user_id, Agent.agent_name == request.agent_name
            )
            ...
            statement = select(
                VectorDB.id, VectorDB.db_type, VectorDB.index_name
            ).where(
                VectorDB.user_id == user_id, VectorDB.index_name == request.index_name
            )
            ...
            if index_result:
                ...
                db_type = (
                    index_result.db_type.value
                )  # Enum value, e.g., DBTypeEnum.pinecone
            ...
            if (
                request.index_name
                and hasattr(request, "index_type")
                and request.index_type
            ):
                # Find the vector DB
                ...
                if vector_db:
                    ...
                    logger.warning(
                        f"Vector DB with name {request.index_name} not found"
                    )
...
def delete_agent(request: AgentDeleteRequest) -> Tuple[Dict[str, str], int]:
    ...
    try:
        ...
        if not request.user_id:
            return {
                "message": "User ID is required. This should be set by the API endpoint."
            }, 400
        ...
        with Session(engine) as session:
            ...
            if not agent:
                return {
                    "message": f"Agent with name '{request.agent_name}' not found"
                }, 404
            ...
            return {
                "message": f"Agent '{request.agent_name}' deleted successfully"
            }, 200
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            statement = (
                ...
                .where(Agent.user_id == user_id_int, Agent.agent_name == agent_name)
            )
            ...
            if db_type == "Pinecone":
                statement = select(PineconeDB.embedding).where(
                    PineconeDB.vector_db_id == db_id
                )
                embedding = session.exec(statement).first()
            elif db_type == "FAISS":
                statement = select(FaissDB.embedding).where(
                    FaissDB.vector_db_id == db_id
                )
                embedding = session.exec(statement).first()
            else:
                embedding = None
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get agent settings: {str(e)}"
        )
...
def get_user_agents(user_id: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            for agent in agents:
                logger.info(
                    f"Processing agent: {agent.agent_name}, ID: {agent.id}, vector_db_id: {agent.vector_db_id}"
                )
                ...
                if agent.vector_db_id:
                    ...
                    if vector_db:
                        logger.info(
                            f"Found vector DB for agent {agent.agent_name}: index_name={vector_db.index_name}, db_type={vector_db.db_type.value}"
                        )
                        agent_data["index_name"] = vector_db.index_name
                        agent_data["index_type"] = vector_db.db_type.value
                    else:
                        logger.warning(
                            f"Vector DB with ID {agent.vector_db_id} not found for agent {agent.agent_name}"
                        )
                else:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get agents for user: {str(e)}"
        )
