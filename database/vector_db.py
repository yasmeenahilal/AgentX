...
def insert_into_vector_db(user_id: str, index_name: str, db_type: str):
    ...
    try:
        with Session(engine) as session:
            ...
            logger.info(f"Inserted into vector_db: user_id={user_id}, index_name={index_name}, db_type={db_type}")
...
def insert_into_pinecone_db(
    ...
    try:
        with Session(engine) as session:
            ...
            logger.info(f"Inserted into pinecone_db: user_id={user_id}, index_name={index_name}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create Pinecone database: {str(e)}"
        )
...
def get_data_from_pinecone_db(user_id: str, index_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get Pinecone database data: {str(e)}"
        )
...
def insert_into_faiss_db(
    ...
    try:
        with Session(engine) as session:
            ...
            logger.info(f"Inserted into faiss_db: user_id={user_id}, index_name={index_name}")
...
def update_file_upload(user_id: str, index_name: str, file_name: str, file_path: str):
    ...
    try:
        with Session(engine) as session:
            ...
            logger.info(f"Updated file_upload: user_id={user_id}, index_name={index_name}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update file upload: {str(e)}"
        )
...
def get_pinecone_api_index_name_type_db(user_id: str, index_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get Pinecone API key: {str(e)}"
        )
...
def delete_pinecone_index_from_db(user_id: str, index_name: str):
    ...
    try:
        with Session(engine) as session:
            ...
            logger.info(f"Deleted index from DB: user_id={user_id}, index_name={index_name}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to delete Pinecone index: {str(e)}"
        )
...
def set_agent_index_to_none(user_id: str, index_name: str):
    ...
    try:
        with Session(engine) as session:
            ...
            if not vector_db:
                logger.warning(f"Vector DB with user_id: {user_id} and index_name: {index_name} not found.")
                return
            ...
            session.commit()
            logger.info(f"Set agent index to None: user_id={user_id}, index_name={index_name}")
            
    except Exception as e:
        logger.error(f"Error setting agent index to None: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to set agent index to None: {str(e)}"
        )
