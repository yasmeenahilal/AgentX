...
async def list_agents_by_id(
    ...
    try:
        ...
        if str(current_user.id) != user_id:
            raise HTTPException(
                status_code=403, detail="You can only access your own agent list"
            )
...
async def create_agent(
    request: AgentCreateRequest, current_user: User = Depends(get_current_active_user)
):
    """
...
async def update_agent(
    request: AgentUpdateRequest, current_user: User = Depends(get_current_active_user)
):
    """
...
async def delete_agent(
    ...
    try:
        ...
        if agent_name and current_user:
            agent_config = get_agent_settings(current_user.id, agent_name)
...
async def query_agent(
    ...
    try:
        ...
        if not agent_id:
            ...
            raise HTTPException(
                status_code=500, detail="Internal error: Agent ID missing."
            )
        ...
        if (
            ...
            or "answer" not in result_dict
            or "session_id" not in result_dict
        ):
            logger.error(
                f"Unexpected result format from rag_app.query_agent: {result_dict}"
            )
            raise HTTPException(
                status_code=500, detail="Internal error processing agent query."
            )
...
async def debug_agent_data(
    ...
    try:
        ...
        if str(current_user.id) != user_id and current_user.role != "admin":
            raise HTTPException(
                status_code=403, detail="You can only access your own debug data"
            )
...
async def get_agent_by_id(
    ...
    try:
        ...
        if str(current_user.id) != user_id:
            raise HTTPException(
                status_code=403, detail="You can only access your own agent details"
            )
